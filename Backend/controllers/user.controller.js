...
module.exports.registerUser = asyncHandler(async (req, res) => {
  ...
  // generate auth token to return to client
  const token = user.generateAuthToken();

  // Also generate an email verification token and send verification email
  try {
    const verificationToken = jwt.sign(
      { id: user._id, userType: "user", purpose: "email-verification" },
      process.env.JWT_SECRET,
      { expiresIn: "15m" }
    );

    const verification_link = `${process.env.CLIENT_URL}/user/verify-email?token=${verificationToken}`;
    ...
    const mailHtml = fillTemplate({
      ...
      note: "For your security, this verification link is valid for only <strong>15 minutes</strong>. If the link expires, you can request a new one from the login page.",
    });

    // fire-and-log: don't fail registration if email sending fails
